...
uint8_t DHT_Read(double *Temperature, double *Humidity)
{
...
// wait for response
  ...
  if(__DHT_STATUS == DHT_Ok){
    ...
    for (i = 0; i < 5; i++){
      for(j = 7: j >= 0; j--){
        ...
        while(!DigitalRead(DHT_Pin)){
          ...
          if(retries > 70){
            __DHT_STATUS = DHT_Error_Timeout;
            j = -1; //stop inner for loop
            i= 5;   //stop outer for loop
    ...
    // check checksum and return Data
    if(__DHT_STATUS == DHT_Ok){
      if(((uint8_t)(buffer[0] + buffer[1] + buffer[2] + buffer[3])) != buffer[4]){
        __DHT_STATUS = DHT_Error_Checksum;
